...
export default function Register() {
  ...
  const [units, setUnits] = useState<
    { id: number; code: string; nama_unit: string }[]
  >([]);
  ...
  const handleValidate = async () => {
    ...
    try {
      // Cek dulu ke eportal apakah sudah terdaftar
      ...
      if (data.valid) {
        ...
        // Simpan unit_kerja untuk Mahasiswa, Dosen, Pegawai
        if (data.data.unitKode && data.data.unitName) {
          // Format dari SIAKAD (unitKode, unitName)
          setUnitKerja({
            kode: data.data.unitKode,
            nama: data.data.unitName,
          });
        } else if (data.data.unit_kerja) {
          // Format dari SIMPEG (unit_kerja.kode, unit_kerja.nama)
          setUnitKerja({
            kode: data.data.unit_kerja.kode,
            nama: data.data.unit_kerja.nama,
          });
        }
  ...
  const handleRegister = async () => {
    if (selectedRole?.key === "Mahasiswa_PMM" && !form.nama_lengkap.trim()) {
    ...
    try {
      const unitNama =
        unitKerja?.nama ??
        units.find((u) => u.code === selectedUnitCode)?.nama_unit ??
        undefined;
                      ...
                      {(selectedRole?.key === "Dosen_Ext" ||
                        ...
                        <div className="space-y-1.5">
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              setSelectedUnitCode(e.target.value || null)
                            }
